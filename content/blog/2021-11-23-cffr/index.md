...
Under the hood, a CFF file is a YAML file. YAML presents the advantage of being
easily understood by any user, and can be also easily converted to another data
serialization languages, such as JSON or XML. This is an example of the minimal
...
so citing software is a matter of giving credit where credit is due. [Steffi
LaZerte](/author/steffi-lazerte/) mentioned some other reasons of why is
important citing R software on [How to Cite R and R
...
In the case of Zotero (a reference management software), the information of the
CITATION.cff file of any repository is detected when adding that repository to a
library from a url.

This handy feature implies that a GitHub repository can effectively behave as a
DOI, ISBN or arXiv ID. Zotero would recognize the author, title, date and other
relevant metadata of the repository, including it on the personal library of the
user.
...
allows to generate codemeta.json files from R packages metadata .
...
Getting started with cffr is pretty easy. There is a main function (and likely
to be the only one you would need on a in-development package) named
[`cff_write()`](https://docs.ropensci.org/cffr/reference/cff_write.html) that
extract the metadata of your package (already included in your DESCRIPTION and
inst/CITATION files), convert it into a CITATION.cff file and validates it
...
It is possible also to create a `cff` object (a regular R list with a custom
printing method included with the package) of any package installed locally on
your machine. In the next example I create a `cff` object for the
...
is not present, GitHub would reuse other keys of the file to auto-generate a
citation reference.

As already mentioned, cffr uses the information of the DESCRIPTION (via the
desc[^8] package) and the inst/CITATION file to extract the metadata. I will
focus now on the comparison of the citation info of rtweet and the information
generated by cffr:
...
We can check that the core information of the rtweet citation has been also
included on the `cff` object. Let's check also some of the fields included on
the DESCRIPTION file of rtweet:
...
On the next chunk I compare it with the corresponding fields on the `cff`
object:
...
Here I show the list of all the valid keys of the CFF schema. Most of them have
an explicit mapping with the fields (or a combination of fields) of the
DESCRIPTION and inst/CITATION files:
...
The cffr package includes also an extensive
...
Once that we have created an `cff` objects, we can check its validity using the
`cff_validate()` function. This function can be with `cff` objects and with
CITATION.cff files, and on error would display some messages that would help us
debugging our object:
...
The validation of the initial `cff` object is satisfactory, as displayed on the
messages. On the second case, where I forced some invalid values using the
...
On the last few months, support of CITATION.cff files has increasingly grown on
the scientific citation ecosystem. The cffr package allows any R-package
developer to easily integrate the citation information with a wide variety of
services via the creation of a CITATION.cff file leveraging on the support
...
awareness on the efforts of software developers.

I would like also to thank [João Martins](https://zambujo.github.io/) and [Scott
Chamberlain](/author/scott-chamberlain/) for thorough reviews, that helps
improving the package and the documentation as well as [Emily
Riederer](https://emilyriederer.netlify.com/) for handling the [review
