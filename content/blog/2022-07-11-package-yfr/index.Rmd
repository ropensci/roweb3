...
Over the `r floor(n_years)` of existence, the SP500 index returned an annual compounded interest of `r scales::percent(ret_comp, accuracy = 0.02)`. This is quite in line with the roughly 8% per year reported in the media.
...
Out of the `r n_tickers` stocks, one was left out due to the high number of missing days. Internally, `yf_get` compares every ticker to a benchmark time series, in this case the SP500 index itself (see input `bench_ticker`). Whenever the proportion of missing days is higher than the default case (`thresh_bad_data = 0.75`), the algorithm drops the ticker from the output. In the end, we are left with just nine stocks.
