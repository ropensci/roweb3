...
```{r, results='asis', eval=(length(posts) > 0)}
cat("## On the blog\n\n")
...
`r snakecase::to_sentence_case(english(length(usecases)))` use case`r if (length(usecases) > 1) "s"` of our packages and resources `r if ) "have" else "has"` been reported since we sent the last newsletter.
