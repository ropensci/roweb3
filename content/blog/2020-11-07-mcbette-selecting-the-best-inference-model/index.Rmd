...
```{r plot_densitree,hugoopts=list(alt="the estimated evolutionary history of primates", caption="the estimated evolutionary history of primates", width=300)}
...
default_model$mcmc <- beautier::create_ns_mcmc(particle_count = 256)
competing_model$mcmc <- beautier::create_ns_mcmc(particle_count = 256)
```
...
```{r est_marg_liks,cache=FALSE,show=FALSE}
marg_liks <- get_test_marg_liks()
```

```{r est_marg_liks,cache=TRUE,eval=FALSE}
...
clock model is heigher (i.e. 0.9343919).
...
```{r,hugoopts=list(alt="the estimated marginal likelihoods", caption="the estimated marginal likelihoods", width=300)}
plot_marg_liks(marg_liks)
```

Note that marginal likeloods can be very close to zero. 
...
email addres (e.g. `mcbette@gmail.com`) that gets 
forgotten by the developer. A user needs a place to ask questions and
submit bug reports. Young researcher may base their next research 
on a newly developed package, only to result in weeks of frustration
due to the packages' maintainters' inconsideration. 
I feel it makes the world a better place,
that rOpenSci enforces its developers to use a website to submit bug reports.

To prove that I am considerate towards my users,
rOpenSci provides me my first critical first users of my packages.
