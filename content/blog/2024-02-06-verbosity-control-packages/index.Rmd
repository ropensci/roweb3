...
```{r}
debug_msg <- function(...) {
  msg <- getOption("mypackage.verbose", "quiet") == "debug"
  if (msg) {
    rlang::local_options(rlib_message_verbosity = "verbose")
    rlang::inform(...)
  }
...
```{r, echo = TRUE, eval = FALSE}
debug_msg("debug message") # not issued
rlang::local_options(rlib_message_verbosity = "verbose")
debug_msg("debug message") # still not issued
rlang::local_options(mypackage.verbose = "debug")
debug_msg("debug message") # now it is
```
```{r, echo = FALSE, eval = TRUE}
debug_msg("debug message") # not issued
suppressMessages(
