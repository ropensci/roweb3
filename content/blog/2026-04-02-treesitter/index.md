...
[ast-grep](https://ast-grep.github.io/) is a useful tool built on Tree-sitter for searching and re-writing code, with a clearer query syntax than Tree-sitter's. Its name is reminiscent of grep, but with ast-grep we do not need to write brittle regular expressions :smile_cat:. [{astgrepr}](https://astgrepr.etiennebacher.com/) by Etienne Bacher is an R wrapper to the Rust bindings of ast-grep, and is used in Etienne's [{flir} package](https://flir.etiennebacher.com/) for [refactoring](https://flir.etiennebacher.com/articles/adding_rules) code.
...
-   Rust CLIs can edit code very fast[^2];
...
[^1]: We could also parse C code with it using [{treesitter.c}](https://sounkou-bioinfo.github.io/treesitter.c/).

[^2]: Rust is a lower level language than R so has less overhead; furthermore this kind of Rust code can be easily parallelized.
