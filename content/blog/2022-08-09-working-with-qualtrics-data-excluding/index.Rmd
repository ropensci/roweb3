...
In the last post, we used the [qualtRics](https://docs.ropensci.org/qualtRics/)[^1] package to import survey data directly from [Qualtrics](https://www.qualtrics.com/) accounts. 
...
If you have particular questions from your specific survey that you want to use to exclude data (for example, attention check questions), you should use regular subset/filter functions from [base R](https://rdrr.io/r/base/subset.html) or [dplyr](https://dplyr.tidyverse.org/reference/filter.html). 
...
Non-anonymous data includes IP address, geolocation, and contact information provided by you (if you have Qualtrics email your respondents). 
...
But you will need to set this up in advance. 
...
Qualtrics records data when your [preview your survey](https://www.qualtrics.com/support/survey-platform/survey-module/preview-survey/) before deploying it. 
You probably always want to remove preview data.
* **Survey progress** (`progress`): 
Qualtrics tracks how much of the survey the respondent completes. 
By default, excluder detects less than 100% completion, but any level of completion percentage can be set to detect.
...
If the data are non-anonymous, IP addresses are available, which can determine the country of origin for the respondent. 
...
excluder detect if the same IP address shows up repeatedly in your data. 
Note, however, that duplicate IP addresses may be legitimately different respondents using the same computer or recycled IP address in the same household, dormitory, or business. 
...
We have now marked which rows meet this metadata criterion.
...
Note that I'm going to show just a subset of columns and rows for clarity (ignore from `dplyr::select()` on).
...
Here we use `check_progress()` to extract the rows with completion percentages less than 100.
...
This leave all rows except the ones that meet the criterion, in this case, the 10 rows out of 100 that have duplicate IP addresses. 
...
excluder has a `deidentify()` function that quickly removes either all metadata columns.
...
As we learned in Part 1 of this post, after connecting to the Qualtrics API, we can quickly (and powerfully) import our Qualtrics data with the `fetch_survey()` function.
...
insightful feedback during [peer review](https://github.com/ropensci/software-review/issues/455) from [rOpenSci](/) editor [Mauro
Lepore](/author/mauro-lepore/) and reviewers [Joseph
