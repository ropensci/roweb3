...
knitr::opts_chunk$set(fig.path = "", comment = "", fig.align = "center")
...
These distributions form the building blocks that piece together advanced statistical models with the wider [probaverse](https://probaverse.com) ecosystem, which is designed to release modellers from low-level coding so production pipelines stay human-friendly.
...
Developed with risk analysis use cases like climate and insurance in mind, these tools translate smoothly to simulations, teaching, and other applied settings.
...
```{r normal, hugoopts=list(alt = "Normal distribution density.")}
dst_norm(0, 1)
plot(dst_norm(0, 1))
```
...
```{r poisson, hugoopts=list(alt = "Poisson distribution probability mass function.")}
dst_pois(3)
plot(dst_pois(3))
```
...
**Empirical distributions**, where the data _are_ the distribution.
...
```{r gamma_empirical_comparison, message = FALSE, warning = FALSE, hugoopts=list(alt="Comparison of Empirical CDF and fitted Gamma CDF for Ozone levels.")}
...
The `stocks` data I've wrangled is a table of _daily percent loss_ for four major European stock indices.
...
```{r empirical_dax, hugoopts=list(alt = "Empirical CDF of DAX stock index daily losses.")}
...
```{r return_period_plot, hugoopts=list(alt="Return Level Plot for Daily Loss Percentages of Stock Indices.")}
return_levels |> 
...
```{r inverse_gamma, hugoopts=list(alt="Hazard function of an Inverse Gamma distribution.")}
