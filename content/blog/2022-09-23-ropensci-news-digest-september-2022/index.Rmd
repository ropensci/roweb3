...
### Improve your code with lintr' new linters!
...
To run _all_ linters there are you could run `lintr::lint_package(linters = lintr::linters_with_tags(tags = NULL))` (or first filter a bit the list returned by `lintr::linters_with_tags(tags = NULL)` in case you see too many [false positives](https://github.com/r-lib/lintr/issues?q=is%3Aissue+is%3Aopen+sort%3Aupdated-desc+label%3Afalse-positive) for one of the linters).
