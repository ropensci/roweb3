...
* **Getting data from several result pages** (example: in gh, automatically implemented in the `gh::gh()` action but done under the hood by https://gh.r-lib.org/reference/gh_next.html);
...
An interesting pattern might be for your package to provide both high level and low level functions, where low level functions can support API calls that the high level functions do not cover (yet).
