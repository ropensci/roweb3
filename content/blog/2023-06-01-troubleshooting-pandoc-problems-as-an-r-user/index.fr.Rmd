...
title: Résolution des problèmes de Pandoc en tant qu'utilisateur de R
author:
- Maëlle Salmon
- Christophe Dervieux
date: '2023-06-01'
...
- quarto
- markdown
description: "Comment résoudre tes problèmes Pandoc grâce à la lecture de documentation,
  l'expérimentation... et un peu d'expérience \U0001F605."
---

L'interface de programmation de Pandoc par [John MacFarlane](https://johnmacfarlane.net/) est un outil très utile : par exemple, Yanina Bellini Saibene, community manager de rOpenSci, a récemment demandé à Maëlle si elle pouvait convertir un document Google en livre Quarto.
...
Pandoc n'est pas seulement utile d'un point de vue anecdotique : il fait partie du programme [R Markdown](https://rmarkdown.rstudio.com/) et permet à [Quarto](https://quarto.org/).
...
Dans ces cas-là, Maëlle, qui a assisté au travail de détective de Christophe\[^1\] dans plusieurs problèmes GitHub, a tendance à se demander "Que ferait Christophe ?".
La réponse est probablement "lire la doc et enquêter".
Dans ce billet, nous rassemblons des ressources et des astuces pour t'aider à dépanner Pandoc en tant qu'utilisateur de R.

\[^1\] : Si tu veux lire des exemples de dépannage, vois ceci. [problème avec rmarkdown](https://github.com/rstudio/rmarkdown/issues/2437) et le problème correspondant [problème Pandoc](https://github.com/jgm/pandoc/issues/8499); ou ceci [Question pandoc](https://github.com/jgm/pandoc/issues/7352).

## Pandoc 101

Disons que nous avons un fichier Markdown et que nous voulons le convertir en HTML.
Nous pouvons le faire à partir de R en appelant la fonction [`pandoc::pandoc_convert()`](https://cderv.github.io/pandoc/reference/pandoc_convert.html)\[^2\] qui enveloppe Pandoc de façon pratique.
Si tu es plutôt un premier utilisateur de terminal, la fonction [`pandoc` outil CLI](https://pandoc.org/MANUAL.html) est ton ami.
Dans la suite de ce billet, nous utiliserons le package R pandoc pour les exemples.

\[^2\] : Le package R pandoc est un développement assez récent. [https://github.com/cderv/pandoc](https://github.com/cderv/pandoc) et permet d'accéder à l'interface de programmation Pandoc à partir de R.
...
Par exemple, nous pourrions modifier légèrement l'appel pour ne pas obtenir d'identifiant pour l'en-tête h1 dans le HTML (c'est-à-dire supprimer l'élément `id` de l'appel `<h1>` ).
...
Si tu fais appel à Pandoc toi-même, directement ou par l'intermédiaire de Christophe. [pandoc de Chri](https://cderv.github.io/pandoc/) de Christophe, tu sauras évidemment que Pandoc est en jeu.

Dans d'autres cas, Pandoc peut être enterré sous des couches d'autres logiciels.
...
- Si tu as un cas de "mais ça marchait hier !", par exemple "je n'ai rien changé du côté de R mais ça ne marche plus : peut-être que la version de Pandoc a changé ? En changeant la version de Pandoc, peut-être en utilisant [la fonctionnalité du paquet pandoc pour gérer la version de Pandoc](https://cderv.github.io/pandoc/reference/index.html#handle-pandoc-version) Un changement de version de Pandoc, par exemple, peut révéler un problème avec une nouvelle version de Pandoc, mais c'est rare.
...
Même si Pandoc est un convertisseur générique qui peut gérer plusieurs formats en entrée et en sortie, dans l'écosystème R Markdown et Quarto, le format d'entrée sera toujours Markdown par le biais d'une fonction `.md` fichier.

Lorsque l'on utilise un `.Rmd` ou d'un `.qmd` le document sera converti en `.md` par knitr avec tout le code et les résultats cousus ensemble.
C'est l'entrée que Pandoc utilisera pour convertir le document en sortie souhaitée.
Le livre de recettes R Markdown de Yihui Xie, Christophe Dervieux et Emily Riederer propose un outil de conversion de données. [diagramme pratique](https://bookdown.org/yihui/rmarkdown-cookbook/rmarkdown-process.html) pour comprendre cette relation.
...
Malheureusement, c'est lorsqu'on commence à dépanner un outil que tous les... *magie* se dévoile 😉 .

Tout d'abord, comment trouver les docs de Pandoc ?
...
Sinon, comment apprendre à "parler Pandoc" ?
...
Par exemple, Maëlle cherchait récemment une option dont elle connaissait l'existence, pour transformer des choses qui ressemblent à des URL en Markdown, disons... `https://ropensci.org` en liens en HTML, par exemple `<a href="https://ropensci.org">https://ropensci.org</a>`.
Elle a d'abord cherché des "liens bruts/nus", avant de (re)découvrir qu'il s'agit d'URI nus, avec l'extension appelée `autolink_bare_uris`.
...
Le format (doh) que Pandoc convertit depuis et vers.
...
[Extensions](https://pandoc.org/MANUAL.html#extensions) modifient le comportement des lecteurs et des rédacteurs de Pandoc.
...
Par exemple, le ["intelligent"](https://pandoc.org/MANUAL.html#extension-smart) interprétera certains éléments typographiques, comme deux tirets, comme en-dash\[^4\], et est activée par défaut pour Markdown.
Donc, si tu veux convertir de Markdown à HTML sans transformer les deux tirets, tu utiliseras `markdown-smart` comme format d'entrée.
Cela signifie que `markdown` *moins* le `smart` extension.
Toutes les extensions ne fonctionnent pas pour un format, alors assure-toi de lire les docs (😉 ). Tu peux utiliser [`pandoc::pandoc_list_extensions()`](https://cderv.github.io/pandoc/reference/pandoc_list_extensions.html) pour lister les extensions disponibles pour un format.
...
pour allumer le *indispensable* emoji.
🎉
...
Vois comment nous nous sommes abstenus de qualifier les extensions d'"options", parce que [options](https://pandoc.org/MANUAL.html#options) c'est autre chose !
...
Ci-dessous, nous utilisons une option pour décaler les niveaux de rubrique de 1, ce qui augmente le numéro de niveau de la rubrique et réduit ainsi l'importance de la rubrique.
...
Ces options peuvent être définies avec Pandoc en utilisant le format YAML à l'aide d'un fichier [fichier par défaut](https://pandoc.org/MANUAL.html#defaults-files).
Il s'agit d'une autre technique avancée que nous n'aborderons pas en détail... mais tu connais maintenant les fichiers par défaut\[^5\].
...
[Variables](https://pandoc.org/MANUAL.html#variables) sont les métadonnées que tu as peut-être l'habitude de transmettre à Pandoc via l'en-tête YAML (pour R Markdown et Quarto).
...
Si tu demandes à Pandoc de créer un slidedeck reveal.js, il le fera en utilisant son modèle par défaut. [reveal.js par déf](https://github.com/jgm/pandoc-templates/blob/master/default.revealjs).
Le modèle est rempli avec ton contenu Markup mais aussi avec des variables comme les noms des auteurs.
Tu peux fournir tes propres [modèles personnalisés](https://pandoc.org/MANUAL.html#templates) en utilisant l'outil `--template` option.
...
Tu peux le protéger de l'analyse Pandoc en l'enveloppant dans un attribut ["attribut raw"](https://pandoc.org/MANUAL.html#generic-raw-attribute).
Ceci est par exemple utilisé par [hugodown](https://github.com/r-lib/hugodown/pull/53) pour protéger les shortcodes Hugo.

Attention, cela fait partie de la `raw_attribute` et peut donc ne pas être incluse par défaut et ne pas être prise en charge par le format que tu as choisi.
...
Ce qui est intéressant, c'est que même si c'est très bien de créer un réprex Pandoc, tu n'es pas obligé de le faire. **ne peut pas** le créer avec reprex, parce que reprex utilise la machinerie R Markdown.

Dans ce billet, nous avons montré quelques exemples, en appelant Pandoc directement sur des chaînes de caractères, ou sur... [des fichiers temporaires créés avec](#pandoc-101).

Voici le répertoire Rmd habituel de Christophe, fourni en tant que RStudio. [snippet](https://support.posit.co/hc/en-us/articles/204463668-Code-Snippets-in-the-RStudio-IDE):
...
Dans tous les cas, comme pour tout reprex, tu voudras le rendre minimal.
...
- Avec Quarto, tu peux accéder au pandoc intégré avec `quarto pandoc` commande.
...
- Avec R Markdown, [`rmarkdown::find_pandoc()`](https://pkgs.rstudio.com/rmarkdown/reference/find_pandoc.html) te permettra de savoir où se trouve le binaire Pandoc.
  Tu peux l'exécuter dans un terminal, ou utiliser la commande [`rmarkdown::pandoc_*` famille de fonctions](https://pkgs.rstudio.com/rmarkdown/reference/index.html#pandoc).
...
- Dans R, le paquet pandoc peut t'aider à exécuter une version spécifique, à comparer entre les versions et à isoler ton travail Pandoc.
  Quel que soit l'outil que tu utilises, une fois que tu connais le numéro de version, tu peux exécuter Pandoc lui-même (à partir de R avec le paquetage pandoc, à partir d'un terminal) en installant la bonne version.

## Suivre l'évolution de Pandoc, ou geler Pandoc !

Suivre l'évolution de Pandoc n'est vraiment pas obligatoire et la lecture du journal des modifications pourrait être difficile à suivre sans une bonne compréhension du fonctionnement de Pandoc.
Toi *pourrais* t'abonner à [Liste de diffusion Pandoc](https://groups.google.com/g/pandoc-announce).
Mais il se peut que tu aies besoin de passer au peigne fin la liste de diffusion de Pandoc. [changelog](https://pandoc.org/releases.html) lorsque tu remarqueras quelque chose d'anormal.
...
- [`pandoc::pandoc_version()`](https://cderv.github.io/pandoc/reference/pandoc_version.html) qui renvoie la version du pandoc par défaut (la version active, généralement la plus récente parmi les versions de pandoc). **installées avec le paquetage R**) ;
...
- [`pandoc::pandoc_rstudio_version()`](https://cderv.github.io/pandoc/reference/rstudio_pandoc.html) qui renvoie la version de Pandoc construite dans l'IDE (il peut s'agir d'une version différente de [`rmarkdown::pandoc_version()`](https://pkgs.rstudio.com/rmarkdown/reference/pandoc_available.html) qui peut être celle de PATH ou de `RSTUDIO_PANDOC` ou de la variable d'environnement).

Alors oui, faire attention à la version de Pandoc demande un peu d'attention.

Avec Quarto, c'est moins un problème qu'avec R Markdown puisque Quarto épinglera une version de Pandoc.
Avec R Markdown, la conception est de bien fonctionner avec la dernière version de Pandoc tout en continuant à fonctionner avec les versions plus anciennes.
...
Conseil particulier si tu construis avec Pandoc sur CI : épingle une version de pandoc pour ta configuration de production (GitHub Actions workflow par exemple) afin qu'elle corresponde à ton environnement de développement local\[^6\].
...
- Si ton problème est lié à Pandoc pour R Markdown ou Quarto en particulier, rends-toi sur la page [forum de la communauté Posit](https://community.rstudio.com/) ou à leurs dépôts Github respectifs.

Et si Pandoc n'est pas suffisant pour ton cas d'utilisation ?

- [Quarto](https://quarto.org/) est en fait une couche très conviviale de Pandoc avec beaucoup de fonctionnalités supplémentaires et de nouveaux paramètres par défaut pour améliorer Pandoc pour la publication scientifique. Donc en général, si Pandoc n'est pas suffisant (ou difficile à utiliser), va voir Quarto (qui a des projets, etc.). Utiliser Quarto avec des fichiers Markdown (.md) revient à utiliser un Pandoc plus intelligent (pas de R, pas de calcul, seulement une conversion de texte).
- Tu pourrais *construire sur* Pandoc. Christophe recommande de se renseigner sur [les filtres Lua](https://pandoc.org/lua-filters.html). Voir aussi [Filtres Lua pour Quarto](https://quarto.org/docs/extensions/filters.html); [Filtres Lua pour R Markdown](https://bookdown.org/yihui/rmarkdown-cookbook/lua-filters.html). Maëlle n'a pas encore appris à les connaître, et a tendance à convertir les choses en HTML puis à les analyser avec xml2\[^7\].
- Tu pourrais ouvrir une demande de fonctionnalité Pandoc.
- Tu peux utiliser autre chose que Pandoc, comme le logiciel [commonmark R de Jeroen Ooms](https://docs.ropensci.org/commonmark/) ou le paquet [markdown de Yihui Xie](https://pkgs.rstudio.com/rmarkdown/articles/rmarkdown.html) ou le paquet [tinkr de Zhian Kamvar](https://docs.ropensci.org/tinkr/) qui s'en inspirent.

\[^7\] : Bien que cela puisse faire en sorte que les flux de travail se sentent comme des [L'incroyable machine](https://en.wikipedia.org/wiki/The_Incredible_Machine_\(video_game\)).

## Conclusion

Dans ce billet, nous avons partagé quelques ressources utiles pour dépanner tes problèmes de Pandoc.
Il est crucial de se rappeler que même si cela peut sembler beaucoup, tes compétences et ton vocabulaire Pandoc s'amélioreront avec la pratique, comme par exemple. [convertir des documents Google](https://docs.ropensci.org/quartificate/) pour un collègue ! :wink :

En tant qu'utilisateur de R, n'oublie pas que Pandoc prend en charge un grand nombre de tes outils de publication ; et qu'il existe un package R pratique pour interagir avec Pandoc : pandoc 🎉.

Si tu aimes jouer avec des fichiers de différents formats, tu pourrais aussi apprécier la lecture de . [rtika](/blog/2018/04/25/rtika-introduction/) par Sasha Goodman.
