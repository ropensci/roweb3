...
tidy_issue <- function(issue) {
  ...
  tibble::tibble(
    label = label,
    title = issue$title,
    holding = "holding" %in% labels,
    others = toString(labels)
  )
}

issues <- purrr::map_df(issues, tidy_issue)
no_holding <- sum(issues$holding)
issues <- dplyr::filter(issues, !holding)
text <- sprintf("There are %s active submissions", nrow(issues))
if (no_holding > 0) {
  text <- paste0(
    text,
    sprintf(
...
no_label <- issues[is.na(issues$label),]
issues <- issues[!is.na(issues$label),]
count_label <- function(label) {
  no <- sum(issues$label == label, na.rm = TRUE)
  url <- paste0("https://github.com/ropensci/software-review/issues?q=is%3Aissue+is%3Aopen+sort%3Aupdated-desc+label%3A", label)
  sprintf("* %s ['%s'](%s):\n\n %s.", no, label, url, paste0(paste0("    * ",issues$title[!is.na(issues$label)][ issues$label == label]), collapse = "\n\n"))
}

cat(text)
cat(
  paste0(
    " Active issues are at different stages: \n\n",
    paste0("* [Editorial team checks](https://github.com/ropensci/software-review/issues?q=is%3Aissue+is%3Aopen+sort%3Aupdated-desc+no%3Alabel):\n\n ", paste0(paste0("    * ",no_label$title), collapse = "\n\n") ,"\n\n"),
    paste0(
      purrr::map_chr(unique(issues$label[!is.na(issues$label)]), count_label),
      collapse = "\n\n"
    )
  )
)
