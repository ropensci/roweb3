...
In their book [Learn AI-Assisted Python Programming](https://www.manning.com/books/learn-ai-assisted-python-programming), Leo Porter and Daniel Zingaro share the attributes of good functions: One clear task to perform, clearly defined behavior, short in number of line of code, clear input and output, general value over specific use.
...
- Following the previews advice, names should be consistent across code base and _name molds_ are a very good tool for that. This molds are patterns in which the elements of a name are combined, for example if you will calculate the maximum value of crop yield, you need to agree if _maximun_ will be _max_ or _maximun_ and if will be at the beggining or at the end of the variable name: should be _maxYield_ or _yieldMax_?  By normalizing how to name things, our code will be more easy to read.     
...
- There are seveal way to write variables names, camelCase style lead to higher accuracy when reading code (Dave Binkley, 2009) and is better for reading the code with screen readers.  
