...
```{r, eval=FALSE}
options(mypackage.quiet = TRUE) # or {rlang} or {withr}
...
The second advantage is particularly relevant here.
Simply replacing any calls to `message()`, `warning()`, or `stop()` with the equivalent `rlang_inform()`, `rlang_warn()`, or `rlang_abort()` functions allows anyone to suppress all messages by specifying, `options(rlib_message_verbosity = "quiet")`.
...
An example of the advantages of this approach would be trying to debug something which ultimately spawns multiple sub-processes.
Each sub-process will generally inherit global options, and so base `options()` for verbose messaging will be passed through to all sub-processes.
The rlang version enables control over whether or not verbosity settings are passed on to sub-processes, and so enables more precise and effective debugging.
Finally, the `rlang::local_options()` function can be used to set `rlib_message_verbosity = "quiet` to make your package (and indeed all others which use this messaging system) quiet in tests, and so remove the often annoying wall of text that appears when running tests.
...
More generally, it may often be useful to implement different levels of verbosity for users and developers, like "quiet", "inform", "debug", and "verbose".
...
## Bonus section: "verbose" or "quiet"?
...
One problem with this is that users may often want to debug only one particular package, while leaving all other packages quiet.
...
pkg_message <- function(...) {
  ...
  if (msg) {
    rlang::with_options(rlib_message_verbosity = "verbose", rlang::inform(...))
  } else {
    rlang::inform(...)
  }
}
...
And to return to the question of this sub-section: Note that the first line of our `pkg_message()` function gets an option called "verbose" and checks whether it is set to "verbose".
The logic of the function is then rather straightforward, and most importantly positive - that is, it requires no negation checks like `if (!msg) ...`.
Rewriting that function with an option "mypackage.quiet" results in definitely more contorted logic.
Whatever your verbosity control, we recommend at least attempting to write a custom message function like the above to ensure that your control logic is ... logical.
We're pretty sure you'll see that "verbosity" makes logical control easier than "quiet".
So make some noise! Just ensure that you can control it!
