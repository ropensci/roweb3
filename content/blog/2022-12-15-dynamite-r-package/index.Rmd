...
In this post, I will illustrate the use of dynamite for a synthetic control approach for time series causal inference, originally suggested by Abadie et al.[^2], and further extended and popularized by Brodersen et al.[^3] and their [CausalImpact](https://CRAN.R-project.org/package=CausalImpact) package. 

The basic idea of the synthetic control method is that you have a time series of interest $y_1,\ldots,y_T$, for example daily sales of some product. 
Then an intervention was made at some time point $t<T$ (e.g., an ad campaign was started). 
Typically you also have one or more "control times series" $x$ which have an effect on $y$ but for which the intervention did not affect (e.g. price and some properties of the product, demographic variables etc.). 
...
Here I'll show how the dynamite package can be used for synthetic control when also the control series are subject to the interventions, by jointly modelling both the main response variable of interest as well as the control time series. While I here focus on a univariate, non-panel data (i.e. we have data on only a single "individual"), the same approach can easily be used for panel data as well.
