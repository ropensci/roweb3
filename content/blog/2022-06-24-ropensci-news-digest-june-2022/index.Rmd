...
present_one <- function(package) {
  ...
  if (package$package == "tidyqpcr") {
    package$onboarding <- "https://github.com/ropensci/software-review/issues/470"
    rev <- "Kelsey Montgomery"
  }
...
format_post <- function(post) {
  ...
  if (post$description != "") {
    ...
    if (url == "/blog/2022/06/16/publicize-api-client-yes-no") {
      string <- paste(string, "This post was discussed on the [R Weekly Highlights podcast](https://share.fireside.fm/episode/87RSVeFz+4qaUu8e0?t=540).")
    }
  } else {
    string <- paste0(string, ".")  
  }
