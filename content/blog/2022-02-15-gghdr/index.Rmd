...
__Before:__
```{r hdrcde-boxplot, echo=TRUE, message=FALSE, warning=FALSE, hugoopts = list(alt = "The plot shows highest density region boxplots split into two disjoint boxes with mode represented by a line in the first box and other regions shaded by different probability coverage.")}
library(hdrcde)
hdr.boxplot(faithful$eruptions,all.modes = FALSE) 
```
...
__After:__
```{r gghdr-boxplot, echo=TRUE, message=FALSE, warning=FALSE, hugoopts = list(alt = "A similar plot as the left one showing highest density regions in ggplot2 framwework so that any other geom can added easily.")}
library(gghdr)
library(ggplot2)
ggplot(faithful, aes(y = eruptions)) +
  geom_hdr_boxplot(all.modes = FALSE) 
```
</div>
</div>
...
__Before:__
```{r hdrcde-den, echo=TRUE, message=FALSE, warning=FALSE, hugoopts = list(alt = "A similar plot as the left one showing highest density regions in ggplot2 framwework so that any other geom can added easily."}
library(hdrcde)
