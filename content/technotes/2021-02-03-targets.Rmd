...
knitr::opts_chunk$set(fig.path = "2021-02-03-targets/", comment = "#>")
...
tar_script({
  ...
  list(
    tar_target(raw_data_file, "2021-02-03-targets/raw_data.csv", format = "file"),
    tar_target(data, read_clean(raw_data_file)),
    tar_target(fit, fit_model(data)),
    tar_target(hist, create_plot(data))
  )
...
```{r, echo = FALSE}
tar_visnetwork() %>%
  visOptions(width = "80%", height = "400px") %>%
  visSave(
    file = "2021-02-03-targets/graph-minimal.html",
...
It is only a matter of time before these backends become ready to [leverage the cloud](https://aws.amazon.com/)[^9].

Your results live in either the [`_targets/` data folder](https://books.rOpenSci.org/targets/files.html#internal-files) or [Amazon S3 buckets](https://books.rOpenSci.org/targets/cloud.html#storage), depending on your preferences.
But no matter where you put the data, you can load it back into R the same way.

```{r, echo = FALSE}
ggsave(
  filename = "2021-02-03-targets/histogram.png",
  plot = tar_read(hist),
  width = 8,
  height = 4,
  bg = "transparent"
...
Up-to-date targets do not rerun, which saves countless hours of runtime in computationally intense fields like machine learning, Bayesian statistics, statistical genomics.
...
<blockquote><p lang="en" dir="ltr">Nearly everything that happens in R results from a function call. Therefore, basic programming centers on creating and refining functions.[^10]</p>&mdash; John Chambers</blockquote>
...
Target factories are reusable shorthand to return whole lists of pre-configured target objects, ideally implemented inside packages.
...
tar_visnetwork() %>%
  visOptions(width = "80%", height = "400px") %>%
  visSave(
    file = "2021-02-03-targets/graph-stantargets.html",
    background = "transparent"
  )
```
