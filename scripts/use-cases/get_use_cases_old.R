...
get_one_page <- function(page) {
  ...
  resp <- httr2::request(sprintf(
    "https://discuss.ropensci.org/search.json?q=category:usecases&page=%s",
    page
  )) |>
    httr2::req_headers(
      'Api-Key' = key,
  ...
  purrr::map(resp$topics, "id") |> unlist() |> as.character()
}

usecases_ids <- unlist(purrr::map(1:4, get_one_page))
length(usecases_ids)
# remove intro and others
usecases_ids <- usecases_ids[
  !usecases_ids %in%
    c(
      33, # intro
      2092, # jsonlite
...
link_resource <- function(resource, packages = get_packages()) {
  ...
  if (resource == "rOpenSci contributing guide book") {
    return(
      "[rOpenSci contributing guide book](https://contributing.ropensci.org)"
    )
  }
...
get_info <- function(id, packages = packages) {
  ...
  list(
    title = topic$title,
    reporter = topic$details$created_by$name %bla%
